...
def cmd_scrape_psychological(args):
    ...
    tickers = args.tickers.split(",") if args.tickers else None
    results = asyncio.run(orchestrator.run_full_pipeline(tickers or ["AAPL", "MSFT", "NVDA", "TSLA", "GOOGL", "AMD", "META"]))
...
def cmd_regime_status(args):
    ...
    orchestrator = asyncio.run(create_psychological_orchestrator())
    tickers = args.tickers.split(",") if args.tickers else ["AAPL", "MSFT", "NVDA", "TSLA", "GOOGL", "AMD", "META"]
