    ...
    def _simulate_single(
        ...
        for year in range(1, input_data.projection_years + 1):
            ...
            if year > sim_n_cap:
                ...
                sim_roic = sim_roic * (1.0 - decay_factor) + input_data.wacc * decay_factor

            fcf = sim_ebit_adj * (1.0 - tax_rate) - reinvestment
            pv_fcf = fcf / ((1.0 + input_data.wacc) ** year)
            projected_fcfs.append(pv_fcf)

        total_pv_fcf = sum(projected_fcfs)
        terminal_fcf = fcf * (1.0 + input_data.terminal_growth)
        terminal_value = terminal_fcf / (input_data.wacc - input_data.terminal_growth)
        pv_terminal = terminal_value / ((1.0 + input_data.wacc) ** input_data.projection_years)

        intrinsic_value = total_pv_fcf + pv_terminal
        eva_spread = input_data.roic - input_data.wacc
