...
CEO approved FinBERT dual-head sentiment model v1 (B-20260824-001 / S-20260824-001). v1 = FinBERT encoder + dual head (3-class + regression s∈[-1,+1]), three Bayesian layers (Laplace head posterior, Beta-Bernoulli reliability, conformal CI), multi-dataset training (PhraseBank/FiQA/SemEval/StockTwits/ hand-graded), fraction-first curriculum, PhraseBank→Glassdoor/Reddit transfer analysis, pre-registered bars (Spearman≥0.70, macro-F1≥0.85, coverage 90%±3, Strux≥0.15). PhraseBank excluded from eval (contamination). v2 Qwen lane gated behind 50-text flip test. Feature branch: feature/sentiment-model-v1. Parallel workers dispatched for data layer + model architecture + dataset audit.
