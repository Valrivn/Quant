...
PIT (point-in-time) universe support:
  ...
  universe as of a given date, eliminating survivorship bias.  Real S&P 500
  constituents from github.com/fja05680/sp500 (102 quarterly snapshots).
  Fixed ETF proxies bypass the PIT filter via ``PIT_ALWAYS_AVAILABLE``.
"""
...
# Annualized dividend-yield estimates per ETF/stock (refreshed from yfinance
# TTM dividends / price). Used ONLY for the fee-coverage measurement, never for
# selection. Loaded from config/dividend_yields.yaml (generated by
# scripts/refresh_dividend_yields.py).
def _load_dividend_yields() -> dict:
    path = Path(__file__).resolve().parents[1] / "config" / "dividend_yields.yaml"
    if not path.exists():
        # Fallback to embedded pre-registered values (auditor baseline)
        return {
            "SPY": 0.0125, "VCSH": 0.045, "VCIT": 0.048, "BIL": 0.035,
            "SHY": 0.046, "SGOV": 0.045, "GLD": 0.0, "IAU": 0.0,
            "VTI": 0.0103, "VB": 0.012, "BND": 0.0407, "MDY": 0.012,
            "IWM": 0.012,
            "JNJ": 0.031, "PG": 0.026, "KO": 0.030, "PEP": 0.031,
            "MCD": 0.028, "CL": 0.026, "KMB": 0.036, "TGT": 0.033,
            "HD": 0.025, "GIS": 0.040, "MO": 0.080, "O": 0.056, "VZ": 0.050,
        }
    with open(path, "r") as fh:
        data = yaml.safe_load(fh)
    return data.get("yields", {})

DIVIDEND_YIELDS = _load_dividend_yields()
